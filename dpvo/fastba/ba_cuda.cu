    ...
    torch::PackedTensorAccessor32<float,1,torch::RestrictPtrTraits> u, const int t0)
{
  ...
  GPU_1D_KERNEL_LOOP(n, ii.size(0)) {
    ...
    Xi[3] = 1.0;
    Jd[0] = -cos(lat) * sin(lon) * d0_inv2;
    Jd[1] = sin(lat) * d0_inv2;
    Jd[2] = -cos(lat) * cos(lon) * d0_inv2;
    Jd[3] = 0.0;
    ...
    const bool in_bounds = (sqrt(rx*rx + ry*ry) < 128) && (d_i > 0.2) &&
...

    {
      ...
      float m3 = -fx * X * xz2;
      // Z*m1-X*m3 = fx
      float Ji[6], Jj[6] = {W*m1, 0.0, W*m3, Y*m3, fx, -Y*m1};

      float Jz = m1 * Jd_tr[0] + m3 * Jd_tr[2];
...

    {
      ...
      float m1 = fy * d2 * X * Y * xz;
      float m2 = -fy * d2 / xz;
      float m3 = fy * d2 * Y * Z * xz;
      // Z*m1-X*m3 = 0
      float Ji[6], Jj[6] = {W*m1, W*m2, W*m3, Y*m3-Z*m2, 0.0, X*m2-Y*m1};
      float Jz = m1 * Jd_tr[0] + m2 * Jd_tr[1] + m3 * Jd_tr[2];
