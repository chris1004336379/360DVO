...
class DPVO:
    def __init__(self, cfg, network, ht=1920, wd=3840, viz=False):
        self.cfg = cfg
        ...
        self.imap_ = torch.zeros(self.mem, self.M, DIM, **kwargs) # context feature（384*1*1）
        self.gmap_ = torch.zeros(self.mem, self.M, 128, self.P, self.P, **kwargs) # matching feature（128*p*p）
    ...
    def update(self):
        ...
        with autocast('cuda', enabled=True):
            with Timer("Correlation", enabled=self.enable_timing):
                corr = self.corr(coords) # correlation volume
            ctx = self.imap[:,self.kk % (self.M * self.mem)]
            with Timer("Updator", enabled=self.enable_timing):
    ...
    def __call__(self, tstamp, image, intrinsics):
        ...
        with Timer("DAS-Feat", enabled=self.enable_timing):
            with autocast('cuda', enabled=self.cfg.MIXED_PRECISION):
        ...
        if self.viser is not None:
            self.viser.update(image_origin, self.poses_, self.points_, self.colors_)
