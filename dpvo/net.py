...
from .sphericalconv import SphericalEncoder
...
class Update(nn.Module):
    ...
    def forward(self, net, hid, corr, flow, ii, jj, kk):
        """ update operator """

        net = net + hid + self.corr(corr)
        net = self.norm(net) 
...
class Patchifier(nn.Module): 
    ...
    def forward(self, images, patches_per_image=96, disps=None, gradient_bias=False, return_color=False, timeit=False):
        ...
        with Timer("SphereResNet", enabled=timeit):
...
class VONet(nn.Module):
    ...
    def forward(self, images, poses, disps, intrinsics, M=1024, STEPS=12, P=1, structure_only=False, rescale=False):
        ...
        fmap, gmap, imap, patches, ix = self.patchify(images, disps=disps, gradient_bias=True)
